            ...
            for UniFam_proteinID in UniFam_proteinIDs:
                if annot.has_key(UniFam_proteinID):
                    ...
                    fout.write('>{} # EC: {} # gene name: {} # product: {} # GO: {} \n'.format(
                        key, newAnnot['ECname'], newAnnot['gene_name'], newAnnot['full_name'], newAnnot['GO']))
                    seq_annotated = True
                    break
        ...
        with open(prod_outfile, 'r') as prod_out:
            for line in prod_out:
                if cls.is_prodigal_header_line(line):
                    ID_CDS_dict = dict()
                    line = line.strip("\n")
                    seq_key_to_val = cls.prodigal_header_to_dict(line)
                    seq_type = cls.get_seq_type(line)
                    is_seq_circular = cls.is_seq_circular(line)
                    contigs_info[seq_key_to_val['seqnum']] = {
                        'seqID': seq_key_to_val['seqhdr'].split()[0],
                        'seqType': seq_type,
                        'circular': 'Y' if is_seq_circular else 'N',
...
def write_rRNA(rrna_list, gbk):
    ...
    for i in xrange(len(rrna_list)):
        ...
        # /note="hit to 16S_rRNA with score 1735.1"
        gbk.write('{0:21}{1:}\n'.format(' ', '/note="hit to' +
                                        rrna_list[i]['attr'] + ' with score ' + str(rrna_list[i]['score']) + '"'))
# =================================================================
# write gbk file for a contig/chromosome/plasmid,
...
def write_tRNA(trna_list, gbk):
    ...
    for i in xrange(len(trna_list)):
        ...
        gbk.write('{0:21}{1:}\n'.format(' ', '/note="tRNA ' + trna_list[i]['Type'] + ' anticodon ' + trna_list[i]['antiCodon'] + ', Cove score ' + str(
            trna_list[i]['CoveScore']) + '"'))  # /note="tRNA Ile anticodon GAT, Cove score 87.85, HMM score 62.4, structure score 25.45"
# =================================================================
...
def prodigalCmd(config):
    ...
    run_off_edge_tag = ' -c' if not runOffEdge else ''
    procedure_name = 'single' if procedure == 'single' else 'meta'
    quiet_tag = ' -q' if quiet else ''
    prodigalCmdPrefix = (f'{prodigalPath} -a {faa}{run_off_edge_tag} -f gbk -m {procedure_name} '
                         f'{quiet_tag} -o {output}')
...
def hmmCmd(config):
    ...
    hmmsearchCmdPrefix = hmmsearchPath + ' -E ' + Eval + ' --noali --cpu ' + cpu + \
        ' --domtblout ' + domtb + ' -o ' + output + ' ' + dataDir + database + '.hmm'
...
def run_tRNAscan(config):
    ...
    tRNAscan_CmdPrefix = "perl -I%s %stRNAscan-SE %s -o %s.tRNA.o -f %s.tRNA.structure -m %s.tRNA.stat -q " % (
        tRNAscanDir, tRNAscanDir, domOpt, workDir + prefix, workDir + prefix, workDir + prefix)
    dotRNAscan = config.getboolean('UniFam', 'dotRNAscan')
    if dotRNAscan:
...
def UniFam(inputfile, config, verbose=False):
    ...
    outputAnnot = os.path.join(workdir, f'{prefix}.annot')  # flat file with annotation for each protein
    outputfaa = os.path.join(workdir, f'{prefix}_annot.faa')  # faa with annotation at the header lines
    readme = open(os.path.join(workdir, 'README'), 'w')  # README file that describes output files
        ...
        if doProdigal:
            ...
            if prodigal_status != 0:
                ...
                readme.write("Prodigal predicted proteins are in file {}\n".format(
                    os.path.basename(config.get('prodigal', 'faa'))))
                readme.write("Prodigal output in gbk format is {}\n".format(
                    os.path.basename(config.get('prodigal', 'prodout'))))
            sys.stdout.write("===== >> {} \n\n".format(str(datetime.now())))  # print current time
        else:
        ...
        if doRNAmmer:
            ...
            if RNAmmer_status != 0:
                ...
                readme.write("RNAmmer output in gff format is {}\n".format(
                    os.path.basename(config.get('RNAmmer', 'rRNAoutput'))))
            sys.stdout.write("===== >> {} \n\n".format(str(datetime.now())))  # print current time
        else:
    ...
    if doParse:
        ...
        parse_domtabfile(domtb, group_file, seq_coverage=config.getfloat(
            'UniFam', 'seqCoverage'), hmm_coverage=config.getfloat('UniFam', 'hmmCoverage'))
